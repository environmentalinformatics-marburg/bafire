...
#' adjacent shadow pixels required to retain a focal shadow pixel as such (ie do 
#' not convert it to non-shadow).
#' 
...
rapid_shadows <- function(x, weights, limit = 0.5, b4 = 4, b5 = 5, ...) {
  ...
  thr <- sapply(lst, "[[", 1)
  ids <- (x[[b4]][] < thr[1]) | (x[[b5]][] < thr[2])
  tmp <- setValues(x[[1]], rep(0, ncell(x[[1]])))
  tmp[ids] <- 1
  
  if (!missing(weights)) {
    vlx <- velox(tmp)
    vlx$meanFocal(weights)
  ...
  overlay(x, if (!missing(weights)) msk else tmp, fun = function(x, y) {
      x[y[] == 1] <- NA
      return(x)
    }, ...)
}
