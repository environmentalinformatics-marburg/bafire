...
#' \code{\link{rapid_sf}}).
#' @param offset \code{integer} offset to UTC in hours (CET = +1; 
#' \url{https://www.esrl.noaa.gov/gmd/grad/solcalc/glossary.html#UTC}).
...
#' @export rapid_ref
#' @name rapid_ref
rapid_ref <- function(x, meta, offset = NULL, digits = NULL, dn = TRUE, ...) {
  ...
  dst <- satellite::calcEarthSunDist(rapidr::rapid_date(meta))
  eai <- c(1997.8, 1863.5, 1560.4, 1395.0, 1124.4)
  
  dt <- rapidr::rapid_date(meta)
  tm <- rapidr::rapid_hour(meta)
  dtm <- as.POSIXct(paste(dt, "00:00:00")) + 3600 * tm
  if (!is.null(offset)) dtm <- dtm + 3600 * offset
  
  lat <- rapidr::rapid_latlon(meta)[1]
  dcl <- satellite::declination(dt, formula = "Spencer")
